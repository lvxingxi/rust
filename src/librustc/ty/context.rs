...
fn _assert_gcx_sync<'tcx>() {
    use ::rustc_data_structures::sync::assert_sync;
    ...
    assert_sync::<FxHashMap<DefIndex,
                         Lrc<FxHashMap<ItemLocalId,
                                       Lrc<StableVec<TraitCandidate>>>>>>();
    assert_sync::<FxHashMap<DefId, Lrc<Vec<Export>>>>();
    assert_sync::<NamedRegionMap>();
    assert_sync::<hir_map::Map<'tcx>>();
    assert_sync::<maps::Maps<'tcx>>();
    assert_sync::<FxHashMap<DefId, Lrc<Vec<hir::Freevar>>>>();
    ...
    assert_sync::<Lrc<OutputFilenames>>();
}
